...
export const getRecommendation = async (userId) => {
  const { data: bookIds, status } = await supabase
    .from('Library')
    .select('bookId')
    .eq('userId', userId);
  ...
  const { data: recommendation } = await supabase
    ...
    .limit(5);
  return {
    recommendation: recommendation.filter((item) => !ids.includes(item.bookId)),
    status,
  };
};
